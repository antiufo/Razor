                               ...
                               IDisposable tagBlockWrapper)
        {
            ...
            if (!seenClose)
            {
                Context.OnError(
                    ...
                    tag.Item1.Content.Length + 1  /* <tagName */);
            }
            else
            {
                if (!isEmpty)
                ...
                if (seenEndScript)
                {
                    ...
                    using (Context.StartBlock(BlockType.Tag))
                    {
                        ...
                        if (!Optional(HtmlSymbolType.CloseAngle))
                        {
                            Context.OnError(
                                tagStart,
                                RazorResources.FormatParseError_UnfinishedTag("script"),
                                length: 8 /* </script */);
                        }
                        Output(SpanKind.Markup);
                    }
        ...
        private bool RemoveTag(Stack<Tuple<HtmlSymbol, SourceLocation>> tags, string tagName, SourceLocation tagStart)
        {
            ...
            if (currentTag != null)
            {
                Context.OnError(
                    ...
                    currentTag.Item1.Content.Length + 1 /* <tagName */);
            }
            else
            {
                Context.OnError(
